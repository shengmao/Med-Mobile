...
- (void)configureView;
//- (void)acesscoredta;
@property NeurospinewatermarkmasterviewViewController *ViewController;
...
- (void)configureView
{
    ...
    if (self.detailItem) {
       ...
       _deseasearray = [[NSMutableArray alloc] init];
        ...
        NSSortDescriptor *sortDescriptor1 = [[NSSortDescriptor alloc] initWithKey:@"shortdescription" ascending:YES];
        sortDescriptors = [[NSArray alloc] initWithObjects:sortDescriptor,sortDescriptor1, nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        ...
        case 1:
        {
            ...
            static NSString *CellIdentifier = @"PatientAllergiecell";
            UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        ...
        case 2:
        {
            ...
            static NSString *CellIdentifier = @"PatientDiagnoseCell";
            UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
            ...
            if (!(_deseasearray == nil || [_deseasearray count]) == 0) {
                //ShortDescription.text = [[[_deseasearray objectAtIndex:indexPath.row] possibleclinicalissue] shortdescription]; 
        ...
        case 4:
        {
            ...
            static NSString *CellIdentifier = @"PatientKlinischeCell";
            UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
            ...
            if (!(_patientissuearray == nil || [_patientissuearray count]) == 0) {
                ...
                NSString *stringFromDate = [formatter stringFromDate:[(Activity *)[_patientissuearray objectAtIndex:indexPath.row]  timestamp]];
                issuedate.text = stringFromDate; 
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    tyxsecdetaiViewController *splitviewController = (tyxsecdetaiViewController *) [nextNavigation.viewControllers objectAtIndex:0];
    splitviewController.detailItem = self.detailItem;
    
    _ViewController = (NeurospinewatermarkmasterviewViewController *)(id)[splitviewController.viewControllers objectAtIndex:0];
    _ViewController.detailItem = self.detailItem;
    _ViewController.ip = _ip;
    
    tyxnewDetailViewController *DetailViewController = (tyxnewDetailViewController *)(id)[splitviewController.viewControllers objectAtIndex:1];
    DetailViewController.detailItem = self.detailItem;
    NSLog(@"prepareforSegue exectution finished");
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSLog(@"---> %s",__PRETTY_FUNCTION__);
     NSLog(@"didselctedRowatindexpath exectution began");
    //jump from current view to next splitview with correspoing detail view
    _ip = [[NSIndexPath alloc] init];
    if (indexPath.section ==0) {
    _ip=[NSIndexPath indexPathForRow:0 inSection:0];
        [self  performSegueWithIdentifier:@"first2second2" sender:self];
    }
    else if (indexPath.section == 1) {
         _ip=[NSIndexPath indexPathForRow:6 inSection:0];
        [self  performSegueWithIdentifier:@"first2second2" sender:self];
        [_ViewController performSegueWithIdentifier:@"Demographie" sender:self];
    }
    else if (indexPath.section ==2) {
        _ip=[NSIndexPath indexPathForRow:2 inSection:0];
        [self  performSegueWithIdentifier:@"first2second2" sender:self];
        [_ViewController performSegueWithIdentifier:@"Diagnostik" sender:self];
    }
    else if (indexPath.section ==3) {
        _ip=[NSIndexPath indexPathForRow:7 inSection:0];
        [self  performSegueWithIdentifier:@"first2second2" sender:self];
        [_ViewController performSegueWithIdentifier:@"Dokumente" sender:self];
     }
    else if (indexPath.section ==4) {
        _ip=[NSIndexPath indexPathForRow:1 inSection:0];
        [self  performSegueWithIdentifier:@"first2second2" sender:self];
        [_ViewController performSegueWithIdentifier:@"Chronik" sender:self];
    }
     NSLog(@"didselctedRowatindexpath exectution finished");
}
